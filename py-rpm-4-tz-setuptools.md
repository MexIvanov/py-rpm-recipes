...
Проект проверяет существование зоны через `timedatectl list-timezones` и получает локальное время этой зоны через команду `date` с подменой переменной окружения `TZ`.
