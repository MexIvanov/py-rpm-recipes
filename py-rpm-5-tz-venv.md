...
В этом примере мы создадим утилиту `rich-tz`. Проект проверяет существование зоны через `timedatectl list-timezones` и получает локальное время этой зоны через команду `date` с подменой переменной окружения `TZ`. Вывод результата осуществляется посредством библиотеки форматированного вывода `Rich`.
...
---
В этом примере мы добавляем файл зависимостей, так как нам нужно, чтобы при сборке RPM-пакета утилита `pip` скачала стороннюю библиотеку `rich` из интернета и положила её внутрь нашего пакета. Логику для простоты мы объединили в один файл.
...
### Структура директории в рабочем пространстве CentOS (`~/rpmbuild/`) для этого проекта
