...
```text
~/rich-weather-1.0/                      # Корневая папка проекта (версия 1.0)
│
...
```bash
ls -lh ~/rpmbuild/RPMS/x86_64/rich-weather-1.0-1.*.rpm
```
...
```bash
sudo dnf localinstall ~/rpmbuild/RPMS/x86_64/rich-weather-1.0-1.*.rpm -y
```
